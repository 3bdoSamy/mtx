...
If your shell exports `NODE_ENV=production`, installer builds still work because the script forces dev dependency installation during build steps. The control engine also keeps `typescript` as a runtime dependency to guarantee `tsc` availability across repeated installs.
